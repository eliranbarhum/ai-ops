...
def normalize_cluster_capacity(data: dict) -> dict:
    ...
    for cluster in data.get("clusters", []):
        # vCenter REST API returns summary stats under resource pool
        # We treat missing values as 0 since we'll get real metrics from vROps
        entities.append({
            "entity_type": "cluster",
            "name": cluster.get("name", "unknown"),
            "cpu_usage": cluster.get("cpu_usage_pct", 0),
            "ram_usage": cluster.get("ram_usage_pct", 0),
            "storage_latency_ms": cluster.get("storage_latency_ms", 0),
            "health_state": _health_from_thresholds(
                cluster.get("cpu_usage_pct", 0),
                cluster.get("ram_usage_pct", 0),
            ),
            "timestamp": ts,
        })
...
def normalize_vrops_metrics(data: dict) -> dict:
    ...
    for resource in data.get("resources", []):
        ...
        ram = resource.get("ram_usage", 0)
        latency = resource.get("storage_latency_ms", 0)
        entities.append({
            "entity_type": "cluster",
...
def _health_from_thresholds(cpu: float, ram: float, latency: float = 0) -> str:
